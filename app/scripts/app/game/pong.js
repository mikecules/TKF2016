...
$app.Pong = function (canvasModalWidget, webGLDrawUtilities) {
  ...
  function _Game() {
    ...
    function __Player(name, position, boundingRect, playerType) {
      ...
      ___player.draw = ___draw;
      ___player.update = ___update;
      ___player.setDirection = ___setDirection;

      ___player.setName = function(name) {
        ___name = name;
        return this;
      };

      ___player.getName = function() {
        return ___name;
      };
    ...
    //////////////////////////////////////////////////////////////////////////////////////////
    // __Map encapsulates our toronto-map (essentially a bunch of rectangles that creates a fence...)
    ...
    function __tick() {
      ...
      if (!__isGamePaused()) {
        ...
        for (count = 0; count < __players.length; count++) {
          ...
          if (!__thePlayer.isHuman() || player !== __thePlayer) {

            // is the robot going to screw up - we randomize it's chance
            var willRobotWillMessUpThisFrame = false;// (1 + Math.floor(Math.random() * 100)) > __robotErrorPercentage ? false : true;
          ...
          if (ballDirection & __DIRECTIONS.NORTH) {
            ...
            ballYToHit = (ballPosition.y - ballRadius * 1.2);
            inPlayerYRange = y2 >= ballYToHit;
            //console.log('Ball to hit ', ballYToHit, inPlayerYRange);
          }
    ...
    __game.on = function(eventName, fn) {
      if (typeof fn === 'function' && typeof __eventCallbacks[eventName] !== 'undefined') {
        __eventCallbacks[eventName] = fn;
      }
